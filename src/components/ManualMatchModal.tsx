...
export const ManualMatchModal: React.FC<ManualMatchModalProps> = ({
  ...
  useEffect(() => {
    if (rawPathOrName) {
      ...
      setCustomTitle(candidateTitle || '');
      setCustomYear(parsedGeneral.year || new Date().getFullYear());

      // Try initial automated match
      handleAutoLookup(candidateTitle, detectedType, resolvedSeason, resolvedEpisode, epInfo.episodeTitle);
    }
  ...
  const handleAutoLookup = async (
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (json.success && json.data) {
          ...
          const newMedia: MediaMetadata = {
            ...
            overview: d.overview || `${titleQuery} synopsis and media package.`,
            tagline: d.tagline || 'Original Media Vault Edition',
            genres: d.genres || (type === 'series' ? ['Drama', 'Series'] : ['Feature Film', 'Cinema']),
            rating: d.rating || 8.5,
            posterUrl:
              type === 'series'
      ...
      setSearchError('Could not auto-match metadata. Please refine the title below.');
    } finally {
      setIsSearching(false);
    }
  };
  ...
  const handleApplyMatch = () => {
    ...
    const finalized: MediaMetadata = {
      ...(previewMedia || {
        ...
        year: customYear,
        overview: `${customTitle} is a ${mediaType === 'series' ? 'TV series' : 'movie'} cataloged in SambaVault.`,
        tagline: 'Custom matched title',
        genres: mediaType === 'series' ? ['Drama', 'TV Series'] : ['Feature Film'],
        rating: 8.5,
        posterUrl:
          mediaType === 'series'
            ? 'https://images.unsplash.com/photo-1522869635100-9f4c5e86aa37?w=800&auto=format&fit=crop&q=80'
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Match unknown folder names, parse Season / Episode numbers, and generate synopses
              </p>
            </div>
          </div>
          <button
          ...
          <form onSubmit={handleManualSearch} className="space-y-4">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
                <span>1. Select Media Type & Enter Official Name</span>
              </label>
                          ...
                          >
                            <div className="flex items-center justify-between">
                              ...
                              <button
                                ...
                                className="p-1 rounded text-slate-500 hover:text-red-400 hover:bg-slate-800 ml-2 cursor-pointer transition"
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
              ...
              >
                ...
                <span>{isSearching ? 'Fetching Synopsis & Metadata...' : 'Search & Fetch AI Synopsis'}</span>
              </button>
            ...
            <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl space-y-3 animate-in fade-in duration-200">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded bg-slate-800 text-slate-300 font-mono">
                  {previewMedia.type.toUpperCase()} • {previewMedia.year}
                </span>
              </div>
              ...
              <div className="flex gap-4">
                ...
                <div className="space-y-1.5 flex-1">
                  <h3 className="text-sm font-bold text-white">{previewMedia.title}</h3>
                  {previewMedia.tagline && (
                    <p className="text-slate-400 italic text-xs font-medium">"{previewMedia.tagline}"</p>
                  )}
