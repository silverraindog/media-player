...
const ENCYCLOPEDIC_CATALOG: Record<string, Partial<MediaMetadata>> = {
  ...
  'ted lasso': {
    ...
    recommendedFolderStructure: 'series/Ted Lasso (2020)/',
...
async function fetchFromTVMaze(cleanTitle: string): Promise<Partial<MediaMetadata> | null> {
  try {
    ...
    return {
      ...
      recommendedFolderStructure: `series/${data.name} (${premieredYear})/`,
      recommendedFilenames: [
...
export async function resolveMediaWithFallback(
  ...
  for (const [key, profile] of Object.entries(ENCYCLOPEDIC_CATALOG)) {
    if (lower === key || (key === '24' && /^24$|^24\b|twenty[\s-]four|jack\s*bauer/i.test(lower))) {
      return {
        ...
        recommendedFolderStructure: profile.recommendedFolderStructure || `series/${profile.title} (${profile.year})/`,
  ...
  if (preferredType === 'series' || preferredType === 'all') {
    ...
    if (tvmazeResult && tvmazeResult.title) {
      return {
        ...
        recommendedFolderStructure: tvmazeResult.recommendedFolderStructure || `series/${tvmazeResult.title} (${tvmazeResult.year})/`,
  ...
  const folder =
    detectedType === 'series'
      ? `series/${cleanTitle} (${effectiveYear})/`
      : detectedType === 'album'
